...
async function init() {
    ...
    // Verificar se quiz_id existe
    if (!quizId) {
        alert("Nenhum questionário disponível para responder.");
        return;
    }
...
function showEmailPrompt(quizId) {
    ...
    document.getElementById("emailForm").addEventListener("submit", (e) => {
        ...
        if (quizId) {
            // Redireciona para responder ao questionário
            window.location.href = `question.html?quiz_id=${quizId}&email=${encodeURIComponent(email)}`;
        } else {
            // Redireciona para criar um questionário
            alert("Nenhum questionário disponível para responder.");
